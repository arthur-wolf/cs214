...
object Server extends IOApp:
  ...
  var boids: Vector[Boid] = Vector.empty
  ...
  def initializeWith(physics: Physics, initialBoids: Vector[Boid]) = synchronized {
    world = World(physics)
    boids = initialBoids
  }
...
case class Config(physics: Physics, initialBoids: Vector[Boid])
...
implicit val decoder: Decoder[Config] = new Decoder[Config]:
  override def apply(c: HCursor): Result[Config] =
    c.downField("physics").as[Physics].flatMap { physics =>
      ...
      bdrs.foldLeft(Right(Vector.empty[Boid]): Result[Vector[Boid]]) { (acc, newRes) =>
        newRes match
          case Left(err) => Left(err)
          case Right(bd) => acc.map(_ :+ Boid(Vector2(bd.x, bd.y), Vector2(bd.vx, bd.vy)))
      }.map(boids => Config(physics, boids))
    }
implicit val configDecoder: EntityDecoder[IO, Config] = jsonOf[IO, Config]

implicit val encoder: Encoder[Vector[Boid]] = new Encoder[Vector[Boid]]:
  def toBoidDataList(boids: Vector[Boid]): Vector[BoidData] =
    boids.map(b => BoidData(b.position.x, b.position.y, b.velocity.x, b.velocity.y))

  override def apply(boids: Vector[Boid]): Json =
    toBoidDataList(boids).asJson

implicit val boidsEncoder: EntityEncoder[IO, Vector[Boid]] = jsonEncoderOf[Vector[Boid]]
