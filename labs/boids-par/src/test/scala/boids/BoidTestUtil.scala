...
def assertEqualsVectorBoid(obtained: Vector[Boid], expected: Vector[Boid], physics: Physics)(using munit.Location) =
...
def assertEqualsBoidVectoruence(obtained: Vector[Boid], expected: Vector[Boid], physics: Physics)(using
    munit.Location
) =
  assertEqualsVectorBoid(obtained, expected, physics)
...
def jsonToBoidVectoruence(boids: ujson.Value): Vector[Boid] =
  boids.arr.map(b => Boid(Vector2(b("x").num, b("y").num), Vector2(b("vx").num, b("vy").num))).toVector
...
def runTestCase(name: String)(using munit.Location) =
  ...
  val initBoids = jsonToBoidVectoruence(testCase("initialBoids"))
  val physics = jsonToPhysics(testCase("physics"))
  testCase("reference")
    .arr
    .map(jsonToBoidVectoruence)
    .toVector
    .drop(1)
    .foldLeft(initBoids) { (world, expected) =>
      val next = tickWorld(world, physics)
      assertEqualsBoidVectoruence(next, expected, physics)
      next
    }

def seqBoidToJson(boids: Vector[Boid]) =
  boids.map(b =>
    ujson.Obj(
      "x" -> b.position.x,
      "y" -> b.position.y,
...
def generateTestCase(name: String, initialBoids: Vector[Boid], physics: Physics, steps: Int = 50) =
  val path = os.pwd / "labs" / "boids" / "src" / "test" / "json" / (name + ".json")
  val reference = Vector.iterate(initialBoids, steps)(boids => tickWorld(boids, physics))
  val testCase = ujson.Obj(
    "initialBoids" -> seqBoidToJson(initialBoids),
    "physics" -> upickle.default.writeJs(physics),
    "reference" -> reference.map(seqBoidToJson)
  )
...
def generateTestCases() =
  ...
  generateTestCase(
    ...
    Vector(Boid(Vector2(501, 305), Vector2(2, 3))),
    noForces
  )
  generateTestCase(
  ...
  generateTestCase(
    ...
    Vector(Boid(Vector2(534, 305), Vector2(-2, 0)), Boid(Vector2(510, 305), Vector2(2, 0))),
    onlyAvoidance
  )
  generateTestCase(
    "12_twoBoidsAvoidanceXY",
    Vector(Boid(Vector2(534, 305), Vector2(-2, 1)), Boid(Vector2(510, 305), Vector2(2, 1))),
    onlyAvoidance
  )
  generateTestCase(
    "13_twoBoidsAvoidanceFar",
    Vector(Boid(Vector2(534, 305), Vector2(-2, 1)), Boid(Vector2(510, 205), Vector2(2, -1))),
    onlyAvoidance
  )
  generateTestCase(
    "14_mixedAvoidance",
  ...
  generateTestCase(
    ...
    Vector(Boid(Vector2(540, 350), Vector2(0, 0)), Boid(Vector2(510, 310), Vector2(0, 0))),
    onlyCohesion
  )
  generateTestCase(
    "21_twoBoidsCohesionDance",
    Vector(Boid(Vector2(540, 350), Vector2(0, 0)), Boid(Vector2(510, 310), Vector2(0, 1))),
    onlyCohesion
  )
  generateTestCase(
    "22_twoBoidsCohesionFar",
    Vector(Boid(Vector2(540, 150), Vector2(0, -1)), Boid(Vector2(510, 310), Vector2(0, 1))),
    onlyCohesion
  )
  ...
  generateTestCase(
    "50_containmentTop",
    Vector(Boid(Vector2(500, 20), Vector2(1, -1))),
    onlyContainment
  )
  generateTestCase(
    "51_containmentBottom",
    Vector(Boid(Vector2(500, 680), Vector2(1, 1))),
    onlyContainment
  )
  generateTestCase(
    "52_containmentLeft",
    Vector(Boid(Vector2(20, 300), Vector2(-1, -1))),
    onlyContainment
  )
  generateTestCase(
    "53_containmentRight",
    Vector(Boid(Vector2(980, 300), Vector2(1, -1))),
    onlyContainment
  )
  generateTestCase(
    "54_containmentCumulative",
    Vector(Boid(Vector2(30, 20), Vector2(-1, -1))),
    onlyContainment
  )
