...
class State(
  val state: Vector[(Card, CardView)],
  val selCardsId: (Option[Int], Option[Int]),
  val showSelCards: Boolean,
  val matching: Boolean,
  val currentPlayer: Int,
  val playing: Boolean,
  val players: Seq[UserId],
  val score: Map[UserId, Seq[Card]]):
  ...
  override def equals(x: Any): Boolean =
    x match
      ...
      case that: State => 
         ...
         showSelCards == that.showSelCards &&
         currentPlayer == that.currentPlayer &&
         matching == that.matching &&
         selCardsId._1 == that.selCardsId._1 &&
         selCardsId._2 == that.selCardsId._2 &&
         state.sameElements(that.state) &&
         score.sameElements(that.score) &&
         players.sameElements(that.players) 
      case _ => false
  
type MemoryState = State // Change this!
