  ...
  extension [A](self: MyLazyList[A])
    ...
    def drop(n: Int): MyLazyList[A] =
      if n <= 0 then self
        ...
        self.state match
          case LZNil => LZNil
          case LZCons(x, xs) => xs.drop(n - 1).state
      )
    ...
    def map[B](f: A => B): MyLazyList[B] =
      MyLazyList(() => 
        self.state match
          case LZNil => LZNil
          case LZCons(elem, tail) => LZCons(f(elem), tail.map(f))
        )
    ...
    def filter(p: A => Boolean): MyLazyList[A] =
      MyLazyList(() => 
        self.state match
          case LZNil => LZNil
          case LZCons(elem, tail) => 
            if p(elem) then LZCons(elem, tail.filter(p))
            else tail.filter(p).state
        )
    ...
    def flatMap[B](f: A => MyLazyList[B]): MyLazyList[B] =
      MyLazyList(() => 
        self.state match
          case LZNil => LZNil
          case LZCons(elem, tail) => f(elem).append(tail.flatMap(f)).state
        )
