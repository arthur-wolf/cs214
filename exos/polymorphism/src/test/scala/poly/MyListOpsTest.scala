...
class ListOpsTests extends munit.FunSuite:
  ...
  test("map: empty list remains empty"):
    assertEquals(map((x: Int) => x)(Nil), Nil)

  test("map: increment integers in the list"):
    assertEquals(map((x: Int) => x + 1)(Cons(1, Cons(2, Cons(3, Nil)))), Cons(2, Cons(3, Cons(4, Nil))))

  test("map: append 's' to strings in the list"):
    assertEquals(map((x: String) => x + "s")(Cons("apple", Cons("banana", Nil))), Cons("apples", Cons("bananas", Nil)))

  test("filter: empty list remains empty"):
    assertEquals(filter((x: Int) => x > 10)(Nil), Nil)

  test("filter: filter even numbers"):
    assertEquals(filter((x: Int) => x % 2 == 0)(Cons(1, Cons(2, Cons(3, Cons(4, Nil))))), Cons(2, Cons(4, Nil)))

  test("filter: filter strings with length greater than 4"):
    assertEquals(filter((x: String) => x.length > 4)(Cons("sky", Cons("banana", Nil))), Cons("banana", Nil))

  test("foldRight: sum of integers"):
    assertEquals(foldRight((x: Int, y: Int) => x + y, 0)(Cons(1, Cons(2, Cons(3, Nil)))), 6)

  test("foldRight: concatenate strings"):
    assertEquals(foldRight((x: String, y: String) => x + y, "")(Cons("a", Cons("b", Cons("c", Nil)))), "abc")

  test("foldRight: multiplication on empty list"):
    assertEquals(foldRight((x: Int, y: Int) => x * y, 1)(Nil), 1)

  test("reduceRight: sum of integers"):
    assertEquals(reduceRight((x: Int, y: Int) => x + y)(Cons(1, Cons(2, Cons(3, Nil)))), 6)

  test("reduceRight: concatenate strings"):
    assertEquals(reduceRight((x: String, y: String) => x + y)(Cons("a", Cons("b", Cons("c", Nil)))), "abc")

  test("forall: all positive numbers"):
    assert(forall((x: Int) => x > 0)(Cons(1, Cons(2, Cons(3, Nil)))))

  test("forall: not all even numbers"):
    assert(!forall((x: Int) => x % 2 == 0)(Cons(1, Cons(2, Cons(3, Nil)))))

  test("exists: contains even number"):
    assert(exists((x: Int) => x % 2 == 0)(Cons(1, Cons(2, Cons(3, Nil)))))

  test("exists: does not contain number greater than 10"):
    assert(!exists((x: Int) => x > 10)(Cons(1, Cons(2, Cons(3, Nil)))))
  ...
  test("zipWith: sum paired integers"):
    assertEquals(
      zipWith((x: Int, y: Int) => x + y)(Cons(1, Cons(2, Cons(3, Nil))), Cons(4, Cons(5, Cons(6, Nil)))),
      Cons(5, Cons(7, Cons(9, Nil)))
    )

  test("zipWith: concatenate paired strings"):
    assertEquals(
      zipWith((x: String, y: String) => x + y)(Cons("a", Cons("b", Nil)), Cons("c", Cons("d", Nil))),
      Cons("ac", Cons("bd", Nil))
    )
  ...
  test("flatMap: empty list"):
    assertEquals(flatMap[Int, Int](n => Cons(n, Cons(n * 2, Nil)))(Nil), Nil)

  test("flatMap: double and flattern a list of integers"):
    assertEquals(
      flatMap[Int, Int](n => Cons(n, Cons(n * 2, Nil)))(Cons(1, Cons(2, Nil))),
