...
object IntTree:
  ...
  object TreeConfig:
    ...
    def Branch(left: TreeConfig, right: TreeConfig): TreeConfig =
      TreeConfig(IntTree.Branch(left.tree, right.tree), op => op(left.reducer(op), right.reducer(op)), op => IntTree.Branch(left.mapper(op), right.mapper(op)))
